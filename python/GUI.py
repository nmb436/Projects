...
class FileTransfer:
    def __init__(self,master):
        ...
        self.button_source = ttk.Button(self.frame, text ='find directory', command = self.askSource)
        self.button_source.grid(row = 1, column = 1)
        self.s = StringVar()
        self.entry_source = Entry(self.frame, textvariable = self.s, width = 40)
        self.entry_source.grid(row = 1, column = 2)
        self.s.set('File Path')
        self.entry_source = self.s.get()
        
        

        self.label_destination = ttk.Label(self.frame, text = 'Destination').grid(row = 2, column = 0)
        self.button_destination = ttk.Button(self.frame, text ='find directory', command = self.askDestination)
        self.button_destination.grid(row = 2, column = 1)
        self.n = StringVar()
        self.entry_destination = Entry(self.frame, textvariable = self.n, width = 40)
        self.entry_destination.grid(row = 2, column = 2)
        self.n.set('File Path')
        self.entry_destination = self.n.get()   

        #go button to execute file transfer
        self.label_go = ttk.Label(self.frame, text = 'transfer files').grid(row = 4, column = 1)
        self.button_go = ttk.Button(self.frame, text ='GO', command = self.file_transfer)
        self.button_go.grid(row = 4, column = 2)
